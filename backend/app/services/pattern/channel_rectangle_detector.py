...
# Width can drift at most this much between the start and the current bar
# before it no longer counts as "parallel" (distinguishes Channel/Rectangle
# from a converging Triangle/Wedge, which use TRIANGLE/WEDGE_MIN_CONVERGENCE_PCT).
MAX_PARALLEL_DRIFT_PCT = 20.0


class ChannelRectangleDetector(BasePatternDetector):
    """
    Rectangle (flat support + flat resistance, price ranging between them)
    and Channel (support/resistance share the same slope — ascending or
    descending — and stay roughly parallel rather than converging, which is
    what separates a channel from a wedge).
    """
    ...
    def detect(self, df: pd.DataFrame, symbol: str, interval: str) -> list[DetectedPattern]:
        cfg = pattern_config
        n = len(df)
        window = df.iloc[max(0, n - cfg.CHANNEL_LOOKBACK_BARS):].reset_index(drop=True)
        if len(window) < 30:
            return []
        ...
        if len(highs) < cfg.CHANNEL_MIN_TOUCHES_PER_SIDE or len(lows) < cfg.CHANNEL_MIN_TOUCHES_PER_SIDE:
            return []
        ...
        if width_start <= 0 or width_now <= 0:
            return []
        drift_pct = abs(width_now - width_start) / width_start * 100
        if drift_pct > MAX_PARALLEL_DRIFT_PCT:
            return []
    ...
    def _classify(res_class: str, sup_class: str) -> str:
        if res_class == "FLAT" and sup_class == "FLAT":
            return "rectangle"
    ...
    def _build(
        ...
        pattern_name = {
            "rectangle": "Rectangle", "ascending_channel": "Ascending Channel",
            "descending_channel": "Descending Channel",
        }[pattern_type]
        is_bullish_bias = pattern_type != "descending_channel"
        direction = PatternDirection.BULLISH if is_bullish_bias else PatternDirection.BEARISH
