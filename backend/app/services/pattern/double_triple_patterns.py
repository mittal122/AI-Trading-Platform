...
class DoubleTriplePatternDetector(BasePatternDetector):
    ...
    def _build(self, df, group: list[SwingPoint], kind: str, symbol, interval, atr) -> DetectedPattern:
        ...
        # Historical anchor — this pattern can complete anywhere in the
        # lookback, so resolve its status against the candles that FOLLOWED
        # it (not today's price, which is meaningless for an old formation).
        status = resolve_forward_status(
            df, group[-1].index, direction, neckline, invalidation_level, atr,
