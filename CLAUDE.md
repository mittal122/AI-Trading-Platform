...
### SMC Analyzer — full section ported from SMC_System_Documentation.pdf (2026-07-05)

User supplied a 32-page spec of a *separate* app ("SMC Crypto Analyzer", a
Java/Spring + React product) and asked to integrate it as a new section here.
Decisions (locked via AskUserQuestion): **port to our Python/FastAPI + React
stack** (not a second Java service, not blended with the existing partial
`pattern/smc_detector.py`), **engine-first phasing**, **new section + sidebar
entry**. Design spec: `docs/superpowers/specs/2026-07-05-smc-analyzer-design.md`;
living task tracker: `docs/SMC_TASKS.md`. Every numeric threshold is from the doc
and lives in `backend/app/core/smc_config.py` — no magic numbers in the engine.

- **Fresh engine** `backend/app/services/smc/` (one file per §5.1 pipeline step):
  `swing.py` (reuses `pattern/swing_detector.py` at lookback 5) · `structure.py`
  (HH/HL/LH/LL, BOS/CHoCH, trend) · `order_blocks.py` (decisive-close mitigation
  only — a wick is the retest, not mitigation) · `fvg.py` · `liquidity.py`
  (EQH/EQL pools + confirmed sweeps: poke beyond + close back within 1-2 bars) ·
  `dealing_range.py` (premium/discount/EQ) · `volume.py` (trendVol) · `atr.py`
  (14-period *simple* mean of Wilder TR, NOT the RMA `ta` variant) · `htf.py`
  (in-memory resample, walk-forward safe) · `poi.py` (POI = unmitigated OB ∩ open
  FVG, inducements, demand/supply) · `scoring.py` (§6.1-6.2 six-factor market-bias
  verdict — informational, never gates a trade) · `confluence.py` (§6.3 per-side
  8-factor + 6-veto checklist — this is what fires a signal at ≥70/110) ·
  `trade_plan.py` (§7 zone-priority entry, structural stop, TP1/TP2, TP1 liquidity
  snap) · `order_flow.py` (§8 depth band + walls + aggTrades CVD; live path only) ·
  `smc_engine.py` (orchestrates all → frozen `AnalysisResult`, ~25ms) ·
  `backtest.py` (§9 walk-forward replay) · `scanner.py` (§13 scheduler service).
  Schemas in `schemas/smc.py`; every step has a `tests/test_smc_*.py` (14 files,
  synthetic exact-assertion + live Binance).
- **Order flow** added `get_raw_order_book` + `get_agg_trades` to
  `BinanceProvider`/`MarketService` (aggTrades were never fetched before — the
  long-noted taker-flow gap, now used for CVD).
- **API** `backend/app/api/v1/smc.py` (prefix `/smc`, registered in `router.py`):
  `POST/GET /smc/analyze`, `POST /smc/backtest`, and the scanner routes
  (`/smc/watchlist` CRUD, `/smc/scanner/settings`, `/smc/scanner/scan`,
  `/smc/signals`, `/smc/signals/{id}/accept|dismiss`). All **public** (like
  `/patterns`) — consistent with this app's unauthenticated frontend.
- **Signal scanner (§13)** — DB tables `smc_watchlist` / `smc_scanner_settings` /
  `smc_signals` (`db/models.py` + `db/repository/smc_repo.py`), **single-operator/
  global** (no `user_id`, matching the paper/live engine singletons — the doc's
  per-user JWT model was deliberately adapted). A 60s **asyncio task in
  `main.py`'s lifespan** runs `scanner_service.scan_once()`: candle-close gated
  (never mid-candle), fires only on confluence≥70 + enabled + weekly cap (2-4) +
  24h dedup. **Accept** opens a paper trade via `ManualPaperTrader.place()`.
  Tables auto-created by `create_tables()` (dev/SQLite); Alembic migration for the
  3 tables added 2026-07-12 (`9e03ebc5e743`) — Postgres path fully covered.
- **Frontend** — new `/smc` page `pages/SmcAnalyzer.tsx` + sidebar entry (◆) +
  `client.ts` types/fns. Components in `components/smc/`: signal banner (hero:
  fired long/short or "no setup"), lightweight-charts with **all overlay layers
  toggleable** (zones, BOS/CHoCH, HH/HL/LH/LL swings, EQH/EQL liquidity lines, EQ
  line, sweeps, inducements, plan levels) + HTF banner, verdict card, both
  long/short trade-plan cards (confluence checklist + reject reasons + one-click
  "Paper-trade this plan"), six bias-factor bars, order-flow panel, backtest panel
  (dependency-free SVG equity curve — recharts is NOT installed here), scanner
  panel, and a **FreezeBar** (frozen vs live price polled 5s, drift %, plan-level-
  cross warning, Re-analyze). Matches the dark/indigo theme exactly.
- **GOTCHAs hit this build**: (1) git repo was **corrupted** (31 zero-byte loose
  objects, bad HEAD) — recovered by moving empties aside + `git fetch origin`
  (the push-after-commit habit saved it). (2) Editing backend files needs a
  uvicorn restart; `pkill`+restart in one compound bash command kept getting the
  new process killed — use the Bash tool's `run_in_background` for a clean detach.
  (3) `grep -c foo && cat >> file` silently skips the append when grep finds 0
  matches (exit 1 short-circuits `&&`) — a following `echo` still runs, so it
  *looks* appended. (4) Dynamic Tailwind class names (`bg-${c}-500`) don't work
  under v4 JIT — use full static strings. (5) `usePersistedState` stores objects
  fine (layer-toggle state).
- **Verified**: all 14 SMC backend tests pass; existing tests unaffected; live
  curl through the running Kronos-loaded server (analyze/backtest/scanner);
  headless-Chrome render of every panel (0 console errors), including toggling
  layers, running a backtest, accepting a scanner signal → paper #1, and placing
  a paper order from a plan. `tsc --noEmit` clean throughout.
- **NOT built (disclosed)**: freehand chart drawing tools (pen/trendline/
...
### Docker optimization pass (2026-07-12, master — redesign branch merged by user)

User merged `ui-ux-redesign` → master via PR #1, then asked for an optimized
dockerization. Setup already existed from Phase 10 — this pass slimmed and
fixed it rather than rebuilt:

- **backend.Dockerfile → multi-stage + CPU-only torch**: builder stage
  (build-essential + pip into `/opt/venv`) → runtime stage copies venv only.
  GPU pins (`nvidia-*`/`cuda-*`/`triton`, ~7GB of CUDA libs, useless without a
  GPU runtime) are grep-filtered out of requirements.txt at build time;
  `torch==2.12.1+cpu` installed from PyTorch's own CPU index
  (`download.pytorch.org/whl/cpu`) instead. **Image: 2.26GB** (was ~10GB).
  Safe because `kronos_service.py:45` already falls back to CPU — verified
  live in-container: Kronos predict 2.6s. No `requirements-docker.txt` second
  file — single source of truth, filter happens in the Dockerfile.
- **Compose YAML bug (pre-existing since Phase 10)**: the `${JWT_SECRET:?...}`
  error message contained `generate with: python ...` — an unquoted `: ` that
  Compose v5's stricter YAML parser rejects ("mapping values are not allowed",
  line 33). Older compose tolerated it. Message moved to a comment.
  GOTCHA: `docker compose build | tail` masks the exit code — the first
  "successful" build had actually failed at YAML parse; always
  `echo "EXIT: $?"` or avoid the pipe.
- **REAL BUG: production frontend build was broken on master** — `npm run
  build` runs `tsc -b`, which failed: `SymbolSearchInput`'s prop is `onCommit`
  but `SmcScannerPanel.tsx:89` + `SmcAnalyzer.tsx:526` passed `onChange`
  (silently swallowed at runtime — Vite dev strips types without checking), +
  an unused `MouseEventParams` import. **`npx tsc --noEmit` passes trivially
  on this repo** — root tsconfig.json only has project references, so it
  compiles zero files; every earlier "tsc --noEmit clean" was a no-op check.
  Use `npx tsc -b --force` to actually typecheck.
- **Missing SMC Alembic migration added** (`9e03ebc5e743`): smc_watchlist /
  smc_scanner_settings / smc_signals via autogenerate against a scratch
  SQLite DB (upgrade head → autogenerate → `alembic check` clean). Container
  `alembic upgrade head` now creates the complete schema on Postgres.
- **.dockerignore tightened** (docs/, design-system/, *.md, scripts, logs,
  kronos/) + **nginx gzip** for JS/CSS/JSON/SVG (~3-4x on the Vite bundle;
  verified `content-encoding: gzip` live). Frontend image 82.7MB.
- **Verified end-to-end on the real stack** (`docker compose up`): all 6
  migrations applied on Postgres 17, Kronos loaded on CPU, SPA + gzip via
  nginx :80, `/market/overview` live data, SMC analyze, strategy signal,
  paper orders, Kronos predict — all through the nginx proxy.
